...
def prep_rating_df(raw_data):
    ''' get data into series, then create data frame from series.
    Huge performance improvement then creating an empty data frame and append row by row of by 
    .loc operations. 
...
def prep_raing_df_slow(user_activity):
...
def content_based():
    ...
    user_activity, items = load_training_data()
        
    print ('started processing raw data')
    st = time.clock()
    
    user_activity = prep_rating_df(user_activity)
    time_end = time.clock()
    
    print 'data processing completed, time taken {}'.format(time_end -st)
